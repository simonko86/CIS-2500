...
{
    struct Room *rooms = malloc(sizeof(struct Room)*7); //create a pointer to rooms of size array of 7 structs (from 0-6)
    ...
    drawHallways(maximumRowsTop, rooms);
    ...
    playGame(maximumRows, maximumRowsTop,rooms);    //play the game
    free(rooms);   //freeMalloc
    return 0;
}
